...
/**
 * Clase para representar gráficamente un Reloj analógico.
 * <p>
 *
 * Esta clase implementa la interfaz VistaReloj, así que sabe como dibujarse en
 * la pantalla, y esperar un segundo.
 *
...
public class VistaRelojAnalogico implements VistaReloj {
    ...
    /**
     * Construye una vista de reloj analógico con un tamaño determinado de 500x500.
     * 
...
/**
 * Utilizamos una clase heredera de la clase <code>JPanel</code> para pintar en
 * la pantalla. Como esta clase no es utilizada por ninguna otra además de la
 * clase <code>RelojAnalogico</code>, la hacemos privada a ésta al no incluir el
...
class PanelDeReloj extends JPanel {
    ...
    /**
     * Pinta las manecillas utilizando la función de la clase de cual es heredera.
     */
    public void pinta() {
        update(getGraphics());
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawOval((ancho - (int) (ancho * 0.8)) / 2, (alto - (int) (alto * 0.8)) / 2, (int) (ancho * 0.8),
                (int) (alto * 0.8));
    ...
    private void dibujaManecilla(Graphics g, int tiempo, int alto, int ancho, double l) {
        ...
        g.drawLine(ancho / 2, alto / 2, (int) (x * l * ancho) / 2 + ancho / 2, (int) (y * l * alto) / 2 + alto / 2);
    }
    ...
    private void dibujaRayitas(Graphics g) {
        ...
        for (i = 0; i < 60; i++) {
            ...
            g.drawLine((int) (x * 0.77 * ancho) / 2 + ancho / 2, (int) (y * 0.77 * alto) / 2 + alto / 2,
                    (int) (x * 0.8 * ancho) / 2 + ancho / 2, (int) (y * 0.8 * alto) / 2 + alto / 2);
        }
        for (i = 0; i < 12; i++) {
            double x = Math.cos(Math.toRadians((double) (i * 30)));
            double y = Math.sin(Math.toRadians((double) (i * 30)));
            g.drawLine((int) (x * 0.73 * ancho) / 2 + ancho / 2, (int) (y * 0.73 * alto) / 2 + alto / 2,
                    (int) (x * 0.8 * ancho) / 2 + ancho / 2, (int) (y * 0.8 * alto) / 2 + alto / 2);
        }
    }
}
